...
def resnet_2D_v1(input_shape, mode='train'):
    bn_axis = 3
    if mode == 'train':
        inputs = Input(shape=input_shape, name='input')
    else:
        inputs = Input(shape=(input_shape[0], None, input_shape[-1]), name='input')
    x1 = Conv2D(64, (7, 7),
                kernel_initializer='orthogonal',
...
def resnet_2D_v2(input_shape, mode='train'):
    bn_axis = 3
    if mode == 'train':
        inputs = Input(shape=input_shape, name='input')
    else:
...
def choose_net(token='thin_resnet'):
    token_backbone = {
        'thin_resnet': resnet_2D_v1,

...
import yaml
import shutil
from tools import y_hat_statistic
...
def _parse_args():
    ...
    default_wrong_class_log_dir = join(root_dir, "wrong_class_log")
    ...
    parser.add_argument('--wrong_class_log_dir', type=str,
                        default=default_wrong_class_log_dir,
                        help="Directory storing the wrong_class_log " +
                             "e.g. many `distribution png` and `wrong_class_log.log`.")
...
def _main(args):
    ...
    try:
        ...
        while (ingestion_is_alive(prediction_dir) and
            ...
            if (num_preds_new > num_preds):
                ...
                output_class_info, y_wrong_percent_per_class, y_wrong_per_class_id = y_hat_statistic(prediction,
                                                                                                     solution)
                output_class_info = "=======================================================prediction {}==========================================================\n".format(
                    num_preds_new) + output_class_info
                wrong_class_info_output += output_class_info

                wrong_class_log_i_path = os.path.join(wrong_class_log_dir, "wrong_class_prediction_{}.log".format(num_preds_new))
                with open(wrong_class_log_i_path, 'w') as f1:
                    f1.write(output_class_info)
                f1.close()

                plt.bar(x=range(solution.shape[1]), height=y_wrong_percent_per_class)
                fig_name = "wrong_class_distribution_predict " + str(num_preds_new) + ".png"
    ...
    ###############################################
    wrong_class_log_path = os.path.join(wrong_class_log_dir, "wrong_class_log_total.log")
